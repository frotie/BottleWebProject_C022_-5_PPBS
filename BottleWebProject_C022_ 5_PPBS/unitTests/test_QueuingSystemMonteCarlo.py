...
class Test_test_QueuingSystemMonteCarlo(unittest.TestCase):
    ...
    # Tests MonteCarlo with queue

    def test_monteCarloMethodWithQueue_ReturnsServedRequestsCount0(self):
        expected = 0
        canalsCount = 1
        intensityFlowOfRequests = 1 
        requestExecutionMinute = 1
        ...
        maxRequestInQueue = 1
        qs = QueuingSystemMonteCarlo(canalsCount, intensityFlowOfRequests, requestExecutionMinute, endTimeMinute)        
        actual = qs.monteCarloMethodWithQueue(maxRequestInQueue)
        self.assertEqual(expected, actual)

    def test_monteCarloMethodWithQueue_ReturnsServedRequestsCount19(self):
        expected = 19
        canalsCount = 2
        intensityFlowOfRequests = 4 
        requestExecutionMinute = 1
        ...
        randomData = [0.433127118442817, 0.147398306969176, 0.140886785250458, 0.624075090867704, 0.842161961673678,
                      ...
                      0.0652560037386257, 0.406926587054207]
        qs = QueuingSystemMonteCarlo(canalsCount, intensityFlowOfRequests, requestExecutionMinute, endTimeMinute)        
        actual = qs.monteCarloMethodWithQueue(maxRequestInQueue,randomData)
        self.assertEqual(expected, actual)    

    def test_monteCarloMethodWithQueue_ReturnsMathematicalExpectation9(self):
            expected = 18.5
            canalsCount = 4
            intensityFlowOfRequests = 4 
            requestExecutionMinute = 1
            ...
            qs = QueuingSystemMonteCarlo(canalsCount, intensityFlowOfRequests, requestExecutionMinute, endTimeMinute)                
            actual = qs.getMathematicalExpectationQSWithQueue(len(randomData), maxRequestInQueue, randomData)
            self.assertEqual(expected, actual)

    def test_getMathematicalExpectationQSWithQueue_RaiseExceptionValueError(self):
        canalsCount = 1
        intensityFlowOfRequests = 1
        requestExecutionMinute = 1
        endTimeMinute = 6
